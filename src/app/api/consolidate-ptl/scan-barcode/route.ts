// src/app/api/consolidate-ptl/scan-barcode/route.ts
//
...
export async function POST(req: Request) {
  try {
    ...
    const color = COLORS.has(String(operatorColor).toUpperCase())
      ...
      : 'YELLOW';
    // Cast once here (validated against COLORS above) instead of `as never`
    // at every Prisma write site below.
    ...
    const result = await runExclusive(() => prismaDispatch.$transaction(async (tx) => {
      ...
      // Scanning the next barcode is what places the previous one — but ONLY
      // for THIS SAME OPERATOR's own previous pending scan. Concurrent
      // operators (different Ranger colours) are fully independent: Blue's
      // scan must never confirm (and dark) Red's still-pending item. Exclude
      // only THIS package's THIS barcode (step 3 handles it) — NOT the same
      // barcode value in another package, so a reassigned barcode's stale
      // pending is still confirmed. Normally exactly one row per operator is
      // pending (one amber contribution per active Ranger colour).
      ...
      // overwritten by a later re-scan of the same barcode under a different
      // colour, or the row's original owner could never auto-confirm it via
      // their own next scan (step 1's query is scoped by operatorColor).
      ...
      if (wasPlaced) {
        ...
        if (!complete) {
          ...
          await tx.consolidationScan.update({
            where: { id: bScan.id },
            data: { placed: false, placedAt: null },
          });
        }
      }
      ...
      // Colours to physically light THIS slot with: green if complete, else
      // every operator currently holding a pending item here (1 or 2 — this
      // is what lets a shared slot show both Red and Blue at once instead of
      // collapsing to whoever scanned last).
      ...
      for (const p of otherPkgs) {
        ...
        if (pprog.complete) {
          ...
          // this same package (shared slot) — confirming THIS operator's stale
          // pending here must not kill that other operator's light.
    ...
    if (result.location && (result.complete || !result.wasPlaced)) {
      const colors = result.complete ? ['GREEN'] : (result.activeColors.length ? result.activeColors : [color]);
      void setLight(result.location.locationNumber, colors);
    }
    for (const n of result.greenLocs) void setLight(n, 'GREEN');
    for (const n of result.offLocs) void setLight(n, 'OFF');
    for (const d of result.dualLocs) void setLight(d.locationNumber, d.colors);
    ...
    return NextResponse.json({
      ...
      color,
      activeColors: result.activeColors, // all operator colours currently pending on this slot (1 or 2)
      ...
      dual: result.dualLocs,   // locationNumbers still lit for ANOTHER operator's live pending item
    });
  } catch (error: unknown) {
