...
const FR0_REGEX = /^(SNXS\d{16}|SBR01\d{15})$/;
  ...
  try {
    ...
    if (!FR0_REGEX.test(scanId)) {
      return NextResponse.json(
        { error: 'Invalid Scan ID format (must be SNXS+16 digits or SBR01+15 digits).' },
        { status: 400 }
      );
    }

    // 2) Check for duplicate
    const existing = await prisma.cLScan.findFirst({
      where: { scanId },
    });
