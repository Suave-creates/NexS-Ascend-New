...
export async function POST(req: Request) {
  ...
  try {
    ...
    // The query targets wms.order_items directly via fully-qualified name,
    ...
    for (let i = 0; i < awbs.length; i += CHUNK_SIZE) {
      ...
      const sql = `SELECT shipping_package_id, COUNT(product_id) AS barcode_count FROM wms.order_items WHERE shipping_package_id IN (${placeholders}) GROUP BY shipping_package_id`;
      const [rows] = await conn.query(sql, chunk);
      allRows.push(...(rows as ShipmentRow[]));
    }

    const foundAwbs = new Set(allRows.map((r) => String((r as any).shipping_package_id)));
