// src/app/api/consolidate-ptl/complete-location/route.ts
//
...
export async function POST(req: Request) {
  try {
    ...
    const outcome = await runExclusive(() => prismaDispatch.$transaction(async (tx) => {
      const location = await tx.location.findUnique({ where: { barcode: locationBarcode }, include: { rack: true } });
      ...
      const scans = await tx.consolidationScan.findMany({
      ...
      // Append-only — survives master-reset, unlike the rows just updated above.
      await tx.consolidationHistory.create({
        data: {
