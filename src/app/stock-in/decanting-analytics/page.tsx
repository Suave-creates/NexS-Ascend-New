...
const API_PATH = '/api/stock-in/decanting-analytics';
const DESTINATION = 'ASRS';
const DESTINATION_LABEL = 'ASRS';
const METRIC_LABEL = 'ASRS';
const FILENAME_SLUG = 'asrs';
const TREND_LOCATION = 'an ASRS tote location';
...
function TrendChart({ rows, destination }: { rows: DailyRow[]; destination: string }) {
  ...
  return (
    <div className="min-h-[280px] w-full overflow-hidden">
      <svg viewBox={`0 0 ${width} ${height}`} className="block h-auto w-full" role="img" aria-label={`Daily ${destination} inbound trend`}>
...
export default function DecantingAnalyticsPage() {
  ...
  const exportCsv = () => {
    ...
    link.download = `${FILENAME_SLUG}-decanting-analytics-${startStamp}-to-${endStamp}.csv`;
    link.click();
    ...
    <div className="mx-auto max-w-[1600px] space-y-5 pb-8">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-zinc-800 pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white">Decanting Analytics</h1>
        ...
        <div className="max-h-[420px] overflow-auto">
          <table className="w-full min-w-[720px] border-collapse text-sm">
            ...
            <tbody className="divide-y divide-zinc-800">
              ...
              )) : <tr><td colSpan={reportItemTypes.length + 2} className="px-4 py-12 text-center text-zinc-400">No decanting movement found for the selected filters.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
