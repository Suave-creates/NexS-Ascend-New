// src/app/asrs/pid-hunter-transfer/page.tsx
'use client';
...
export default function LocationTransferPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {banner && (
        <Alert tone={banner.type} className="flex items-center justify-between gap-3">
          <span>{banner.text}</span>
          <button
            type="button"
            className="shrink-0 rounded p-1 opacity-55 transition hover:opacity-100"
          ...
          <CardHeader className="flex-col items-stretch gap-4 border-b-0 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h2 className="text-base font-semibold text-brand-700">
                Records at{' '}
                <Badge tone="gold" className="ml-1 font-mono">
                  {loadedLocation}
                </Badge>
              </h2>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} size="sm">
        ...
        <p className="mb-5 text-sm leading-relaxed text-gray-700">
          ...
          <span className="font-medium text-danger-600">
            These records will be deleted from <code>scanned_barcode_inventory</code>. This
            cannot be undone.
          </span>
        </p>
