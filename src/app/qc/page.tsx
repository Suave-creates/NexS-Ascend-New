...
export default function QcPage() {
  ...
  const loadReasons = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/qc/reasons');
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/qc/stats?qcPerson=${encodeURIComponent(qcPerson)}`);
  ...
  const loadBarcode = useCallback(
    async (barcode: string) => {
      ...
      setPrev(null);
      try {
        const res = await fetch(`/api/qc?barcode=${encodeURIComponent(barcode)}`);
        if (res.ok) {
          const data = await res.json();
